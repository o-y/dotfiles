...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // 0.35 = speed multiplier
  float adjusted_iTime = iTime * 0.45;
  vec2 puv = floor(uv * vec2(60., 60. * ratio)) / 60.;
  puv +=
      (smoothstep(0., 0.7, noise2D(puv)) - 0.5) * 0.05 - vec2(0.0, adjusted_iTime * 0.08);

  uv = fract(vec2(uv.x, uv.y * ratio) * 10.);
  float d = roundRectSDF((sd + 0.01) * (uv - .5), sdh, 0.075),
        d2 = roundRectSDF((sd + 0.065) * (fract(uv * 6.) - .5), sdh, 0.2),
        noiseTime = adjusted_iTime * 0.03, noise = snoise(vec3(puv, noiseTime));
  ...
  // 0.8 = opacity
  fragColor = vec4(
      ghosttyCol.rgb + mix(col4, fcol, smoothstep(fw, -fw, d) * smoothstep(fw, -fw, d2)) * 0.25,
      ghosttyCol.a
  );
}